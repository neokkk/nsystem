...
static sensor_data_t *the_sensor_info = NULL;
...
void *watchdog_thread(void* arg) {
  ...
  while (1) {
    mqretcode = (int)mq_receive(watchdog_queue, (void *)&msg, sizeof(msg), 0);
    assert(mqretcode >= 0);
...
void *monitor_thread(void* arg) {
  ...
  while (1) {
    mqretcode = (int)mq_receive(monitor_queue, (void *)&msg, sizeof(msg), 0);
    assert(mqretcode >= 0);
    ...
    if (msg.msg_type == SENSOR_DATA) {
      shmid = msg.param1;
      sensor_data_t *sensor_info = shm_attach(shmid);
...
void *camera_service_thread(void* arg) {
  ...
  while (1) {
    mqretcode = (int)mq_receive(camera_queue, (void *)&msg, sizeof(msg), 0);
    assert(mqretcode >= 0);
